...
$VVALOR=$_GET['idclie'];
 $NOMBRE=$_GET['nombre'];
  ...
  $sql="UPDATE problema SET nombreProblema='$NOMBRE'  where idproblema=$VVALOR";
...
$result= mysql_query("select idproblema,nombreProblema from problema where idproblema=$valor");
while ($row=mysql_fetch_array($result)) {
 $idple=$row['idproblema'];
 $nombre=$row['nombreProblema'];
...
<div class="row">
  ...
  <div class="col-xs-6"><input type="text" name="nombre"  value="<?=$nombre?>" class="form-control" ></div>
   ...
   <input  name="idclie" type='hidden' value="<?=$idple?>">
